...
lazy_static! {
    ...
    //  x86_64
    // a) The first 192MB of host shared heap are initialized.
    // b) The first 1200MB of guest heap are initialized.
    ...
    static ref HEAP_ACCESSED: [QRwLock<u64>; 10] = [
        ...
        #[cfg(target_arch = "aarch64")]
        QRwLock::new(0xFFC0_0000_0000_0000),
        #[cfg(target_arch = "aarch64")]
...
fn _try_gpa_range_set_fmap(entry: usize, mask: u128, gpa_address: u64, npages: u64)
    ...
    if try_set {
        ...
        if (*bucket & mask) != 0u128 {
            ...
            #[cfg(target_arch = "aarch64")]
            let res = _tee::rsi::tee_try_gpa_range_set(gpa_address, npages, true);
            if res.is_ok() {
                *bucket |= mask;
            }
            return res;
...
fn _try_gpa_range_set_heap(entry: usize, mask: u64, gpa_address: u64, npages: u64, to_prv: bool)
    ...
    if try_set {
        ...
        if (*bucket & mask) != 0u64 {
            ...
            #[cfg(target_arch = "aarch64")]
            let res = _tee::rsi::tee_try_gpa_range_set(gpa_address, npages, !to_prv);
            if res.is_ok() {
                *bucket |= mask;
            }
            return res;
